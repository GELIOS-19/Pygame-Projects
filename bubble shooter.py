...
def print_to_display(
    ...
    text_parameters = pygame.font.SysFont(
...
class Bubble(object):
    ...
    def draw(self, win):
        pygame.draw.circle(
            win, self.color, (int(self.x), int(self.y)), self.radius, 0
        )
...
class Player:
    ...
    def draw(self, win):
        pygame.draw.rect(
            win,
            self.player_color,
            (self.player_x, self.player_y, self.width, self.height),
        )
        pygame.draw.rect(
            win,
        ...
        pygame.draw.line(
            win, RED, self.hitbox_gun_start, self.hitbox_gun_end, 2
        )
...
quit_button = Button(
    "Quit", 40, False, True, BLACK, 525, 270, 150, 150, RED, DARK_RED
)
...
while Game:
    ...
    if bubble.offset == 0:
        bubbles.append(
            Bubble((random.randint(0, 89), random.randint(0, 37), 105))
        )
    if len(bubbles) == 0:
        bubbles.append(
            Bubble((random.randint(0, 89), random.randint(0, 37), 105))
    ...
    for bullet in bullets:
        ...
        for index, bubble in enumerate(bubbles):
            if (
                bubble.hitbox[0]
                < bullet.x
                < bubble.hitbox[0] + bubble.hitbox[2]
                and bubble.hitbox[1]
                < bullet.y
                < bubble.hitbox[1] + bubble.hitbox[3]
            ):
    ...
    if (
        ...
        and bubble.hitbox[0]
        <= player.hitbox_player[0] + player.hitbox_player[2]
        and bubble.hitbox[1] + bubble.hitbox[3] >= player.hitbox_player[1]
        and bubble.hitbox[1]
        <= player.hitbox_player[1] + player.hitbox_player[3]
    ):
        print("hi")
